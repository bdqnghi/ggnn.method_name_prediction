...
class DenseGGNNModel():
    ...
    def compute_nodes_representation(self):
        ...
        h = tf.layers.dense(inputs=h, units=50,activation=tf.nn.tanh, use_bias=False)
        h = tf.layers.dense(inputs=h, units=self.node_dim,activation=tf.nn.tanh, use_bias=False)
